  ...
  useEffect(() => {
    const fetchVendorData = async () => {
      try {
        ...
        const vendorsCollectionRef = collection(
          db,
...
const OrdersTab: FC = () => {
    ...
    const fetchOrders = async () => {
      try {
        const ordersCollectionRef = collection(
          db,
          `artifacts/${appId}/public/data/vendors/${vendorId}/orders`
        );
  ...
  const handleCompleteOrder = async (orderId: string) => {
    ...
    try {
      ...
      setStatusMessage(
        `✅ Order ${orderId.substring(0, 8)}... marked as Complete.`
      );
    } catch (e: any) {
                ...
                <tr key={order.id}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    ...
                    <div className="text-sm text-gray-500">
                      {order.hostel}, Room: {order.room_num}
                    </div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span
  ...
  useEffect(() => {
    ...
    const fetchDashboardData = async () => {
      try {
        ...
        setError(
          "Failed to load dashboard data. A 'timestamp' field might be missing on some orders."
        );
      } finally {
        setLoading(false);
      }
...
const MenuTab: FC = () => {
    ...
    const fetchMenu = async () => {
      try {
        const menuCollectionRef = collection(
          db,
          `artifacts/${appId}/public/data/vendors/${vendorId}/menu`
        );
  ...
  const handleDeleteItem = async (id: string) => {
    if (!vendorId || !appId) {
      setError("Cannot delete item: vendor not identified.");
      return;
    }
    ...
    try {
      const itemDocRef = doc(
        db,
        `artifacts/${appId}/public/data/vendors/${vendorId}/menu/${id}`
      );
  ...
  const handleAddItem = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (newItem.name && newItem.description && newItem.price) {
      try {
        ...
        setMenuItems((prev) => [
          ...prev,
          { id: docRef.id, ...newItem, price: priceAsNumber },
        ]);
          ...
          {menuItems.map((item) => (
            <div key={item.id} className="bg-gray-50 rounded-lg p-5 shadow-sm">
              ...
              <button
                ...
                className="mt-4 w-full px-3 py-1 rounded-md text-sm font-medium text-white bg-red-600 hover:bg-red-700"
              >
                Delete
              </button>
            </div>
    ...
    <div className="flex flex-col min-h-screen bg-gray-100">
      ...
      <div className="flex flex-1">
        ...
        <aside
          ...
          } lg:relative lg:translate-x-0 transition-transform duration-300 ease-in-out w-64 bg-white shadow-lg z-30`}
        >
          ...
          <nav className="mt-8 space-y-2 px-4">
            {sidebarItems.map((name) => (
              <button
                ...
                className={`w-full text-left py-2.5 px-4 rounded-lg transition-colors duration-200 ${
                  activeTab === name
            ...
            <button
              onClick={handleLogout}
              className="w-full text-left py-2.5 px-4 rounded-lg text-red-600 hover:bg-red-50 font-bold"
            >
              Logout
            </button>
          </nav>
